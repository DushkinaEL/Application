...
class RatingDonutView @JvmOverloads constructor(context: Context, attributeSet: AttributeSet? = null): View(context, attributeSet) {
    ...
    private fun initPaint() {
        ...
        strokePaint = Paint().apply {
            ...
            strokeWidth = stroke
            // Цвет мы тоже будем получать специальном методе, потому что в зависимости от рейтинга, мы будем менять цвет нашего кольца
    ...
    private fun drawRating(canvas: Canvas) {
        ...
        // Рисуем задний фон ( позже отрисовать в bitmap, так как статичный)
        canvas.drawCircle(0f, 0f, radius, circlePaint)
        //Рисуем "арки", из них будет состоять наше кольцо + специальный метод
